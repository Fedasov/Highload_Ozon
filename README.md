...
![photo_2024-10-14_15-22-51.jpg](images/photo_2024-10-14_15-22-51.jpg)
...
**Profile_Private**
```
  id(16) + phone_number(19) + email(32) + gender(2) + date_of_birth(8) + password_hash(64) + creating(8) = 149
``` 

**Session**
```
  token(32) + profile_id(16) + creating(8) + last_access(8) + user_agent(16) + ip_address(4)= 84
``` 

**Seller**
```
   id(16) + profile_id(16) + creating(8) + rating_number(4) + rating_sum(4) = 48
```

**Seller_Product**
```
   id(16) + seller_id(16) + profile_id(16) + creating(8) = 56
``` 

**Comment**
```
  id(16) + profile_id(16) + product_id(16) + comment(256) + rating(2) + photo(64) + creating(8) = 378
  id(16) + profile_id(16) + product_id(16) + comment(0) + rating(2) + photo(0) + creating(8)= 58
``` 

**Product**
```
  id(16) + title(128) + description(1024) + producr_size(4) + amount(4) + price(10) + imgsrc(16) + category(16) + creating(8) + rating_number(4) + rating_sum(4) = 1 234
``` 

**Elastic_Search**
```
  id(16) + product_id(16) + name(128) + description(1024) = 1 184
``` 

**Category**
```
  id(16) + title(64) + creating(8) = 88
``` 
 
**File_Product**
```
  id(16) + product_id(16) + imgsrc(64) + creating(8) = 104
```

**Shopping_Cart**
```
  id(16) + profile_id(16) + creating(8) = 40
``` 

**Shopping_Cart_Product**
```
  id(16) + shopping_cart(16) + product_id(16) + creating(8) = 56
``` 

**Order_Item**
```
  id(16) + profile_id(16) + track_id(16) + creating(8) = 56
``` 

**Order_Item_Product**
```
  id(16) + order_item_id(16) + product_id(16)= 48
``` 
...
**List_Pick-up**
```
  id(16) + city(16) + street(16) + house(16) + creating(8) = 72
``` 
...
|---------------------------|-----------------|----------------------------------|----------------|
| **Profile_Public**        | 192             | 100 * 10^6                       | 17.9 Гб        |
| **Profile_Private**       | 149             | 100 * 10^6                       | 13.9 Гб        |
| **Session**               | 84              | 56 * 10^6                        | 4.4 Гб         |
| **Seller**                | 48              | 400 * 10^3                       | 18.3 Гб        |
| **Seller_Product**        | 56              | 250 * 10^6                       | 13 Гб          |
| **Comment**               | 378; 58         | 3 * 10^9 * 0,07; 3 * 10^9 * 0,15 | 74 Гб; 24.3 Гб |
| **Product**               | 1234            | 250 * 10^6                       | 287.3 Гб       |
| **Elastic_Search**        | 1184            | 250 * 10^6                       | 275.7 Гб       |
| **Category**              | 88              | 24                               | 20 Кб          |
| **File_Product**          | 104             | 250 * 10^6 * 7                   | 169.5 Гб       |
| **Shopping_Cart**         | 40              | 3.1 * 21 * 10^6 (DAU) * 14       | 34 Гб          |
| **Shopping_Cart_Product** | 56              | 3.1 * 21 * 10^6 (DAU) * 14       | 47.5 Гб        |
| **Order_Item**            | 56              | 3 * 10^9                         | 156,5 Гб       |
| **Order_Item_Product**    | 48              | 3 * 10^9                         | 134 Гб         |
| **Tracking**              | 390             | 966 * 10^6 / 365 * 4 дня         | 4 Гб           |
| **List_Pick-up**          | 72              | 16 000                           | 1.1 Mб         |
...
* PostgreSQL. Реляционные таблицы с отношениями и строгой структурой данных. PostgreSQL предоставляет мощные возможности для работы с транзакциями и обеспечит целостность данных (например, связи между профилями, товарами, заказами и прочим)
...
--------| -----------
Session | Redis      
Profile_Public, Profile_Private, Seller, Seller_Product, Comment, Product, Category, Shopping_Cart, Shopping_Cart_Product, Order_Item, Order_Item_Product, Tracking, List_Pick-up | PostgreSQL 
...
-----------------------|------------------------------------------------| -----------------
Profile_Public         | profile_id                                     | ускорить поиск пользователей по profile_id  
Profile_Private        | phone_number <br/> email                       | ускорить поиск пользователей по номеру телефона <br/> ускорить поиск пользователей по номеру адресу электронной почты 
...
Seller                 | product_id                                     | для быстрого доступа к профилю продавца
Seller_Product         | product_id <br/> saller_id                     | для быстрого доступа к профилю продавца <br/> для быстрого доступа ко всем товарам продавца
...
Elastic_Search         | name <br/> description                         | для быстрого полнотекстового поиска по товарам
Category               | title                                          | для поиска категорий по названию
File_Product           | imgsrc                                         | для быстрого доступа к файлам по их ссылкам
Shopping_Cart          | profile_id                                     | для поиска id корзины пользователя
Shopping_Cart_Product  | shopping_cart_id                               | для поиска всех товаров в корзине пользователя
Order_Item             | profile_id <br/> track_id                      | для поиска всех заказов пользователя <br/> для поиска заказа по трэк номеру
Order_Item_Product     | order_item_id                                  | для поиска всех товаров в заказе пользователя
Tracking               | track_number                                   | для поиска заказа по трэк номеру
...
1) PostgreSQL:
* Шардирование по товарам (Product, Seller, Comment): Шардирование по product_id. Это позволит распределить нагрузку, так как данные товаров (включая продавцов и отзывы) часто запрашиваются вместе.
