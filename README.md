...
Итого                         | Покупатель: 45 023 ГБ ≈ 45 ТБ  <br/>  Продавец: 121.7 ГБ                                                                                                                    | 0.627 <br/> 0.0014 | 1.88 <br/> 0.0042 | 2.5 <br/> 0.0056                  |
...
|--------------------------|---------------------------------------|-----|--------|------------|
| Session                  | 4 + 486 + 1 944 + 3 111 + 486 = 6 030 | 121 | 10 Gb  | 1 Gbit/s   |
| Profile                  | 4                                     | 1   | 10 Mb  | 1 Mbit/s   |
| SallerProduct            | 486                                   | 10  | 500 Mb | 100 Mbit/s |
| Comment                  | 1 944                                 | 39  | 2 Gb   | 500 Mbit/s |
| ShoppingCart             | 3 014 + 97 = 3 111                    | 63  | 3 Gb   | 1 Gbit/s   |
| OrderItem                | 126 + 126 + 486 = 738                 | 15  | 700 Mb | 100 Mbit/s |
| Product                  | 11 668                                | 234 | 11 Gb  | 2 Gbit/s   |
| SearchService            | 1 555                                 | 31  | 1.5 Gb | 500 Mbit/s |
...
В системе будет использоваться облачная модель в связке с оркестрацией Kubernetes
...
Однако проектам использующим окрестрацию все равно приходится держать часть сервисов на виртуалках при условии самостоятельного разветртывания Kubernetes, баз данных и балансировщиков. Поэтому облачная модель развертывания будет намного удобнее, так как вы можете пользоваться managed услугами и в таком случае достаточно комбинации managed Kubernetes + managed DB + cloud monitoring.

### Модель хостинга

Yandex Cloud минимально-достаточный набор managed-сервисов, бесплатная сеть подходит для большинства небольших и среднего размера проектов

Преимущества Yandex Cloud по сравнению с другими сервисами:
1. Простота интерфейса и удобство использования.
2. Цены. По сравнению с Amazon AWS или Google Cloud, Yandex Cloud предлагает более низкие цены.
3. Масштабируемость (для быстрорастущих проектов).
4. Бесплатный период использования.
6. Данные хранятся на территории России, что может быть важно для ряда организаций при соблюдении законодательства о личных данных. К примеру API РЖД не работает за границей [[39]](https://habr.com/ru/articles/790326/)
7. Бесплатная сеть.
