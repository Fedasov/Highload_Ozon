...
3.1 * (6.3 (кБ) + 7 * 86 (кБ)) + 2 * (0.06 (кБ) + 3 * 86 (кБ)) + 3 * (6.3 (кБ) + 7 * 86 (кБ)) + 0.5 * (6.3 (кБ) + 86 (кБ)) = 3756.8 кБ = 4.2МБ
...
7. Трафик на одного пользователя в день:
...
Итого                         | Покупатель: 45 023 ГБ ≈ 45 ТБ  <br/>  Продавец: 121.7 ГБ                          | 0.629 <br/> 0.0014 | 1.89 <br/> 0.0042 | 2.51 <br/> 0.0056                 |
...
![normalized_database_schema.png](images/normalized_database_schema.png)
...
Comment                | product_id <br/> profile_id              | для получения всех комментариев к продукту. <br/> для отслеживания комментариев от конкретного пользователя <br/> В таблице Profile_Comment_Shard будут лежать profile_id и номера шардов, в которых лежат комментарии данного пользователя. Это может с оптимизировать запросы.
...
Стандарт отказоустойчивости датацентров выберем TIER III.
TIER III датацентры гарантируют 99,982% доступность, что означает менее 1,6 часов простоя в течение года.
Хотя уровень TIER IV может предложить еще большую отказоустойчивость, он также влечет за собой значительно большие затраты. TIER III предлагает хороший баланс между отказоустойчивостью и стоимостью
...
# <a name="11"></a> 11. Список серверов
...
* Пиковое значение сетевого трафика: 2.5 Гбит/с
...
```
Округляем до 1 серверa.
Расчет по сетевому трафику: 1 сервер может обрабатывать до 40 Гбит/с.
Для сетевого трафика:
```
2.5Гбит/с / 40Гбит/с = 0.06 серверов
```
Округляем до 1 сервера.
Таким образом, для веб-сервера Nginx потребуется 1 сервера (по RPS).
При расчёте количества на один ДЦ учтём, что для каждого сервера необходимо иметь 1 резервный. С учетом 4 ДЦ получаем следующее распределение:
...
В зависимости от сложности сервиса, он будет занимать разное количество ресурсов. 
К примеру сервис Session, в котором содержится легкая бизнес логика (достать значение по id) должен пропускать много запросов, но занимать не так много RAM, а сервис Search наоборот имеет более тяжелую бизнес логику (кэш + ElasticSearch + ML), и ему требуется больше CPU и RAM 
* Session и ProxyServer Nginx&&Kafka имеют легкое JSON API 1000 RPS - 20 Mb RAM и 10 CPU (100 RPS на 1 CPU)
* Profile, SallerProduct, Comment, ShoppingCart, OrderItem, Product, AppIndever имеют среднюю бизнес-логику 1000 RPS - 100 Mb RAM и 20 CPU (50 RPS на 1 CPU)
* Search более тяжелая бизнес-логика 1000 RPS - 1000 Mb RAM и 40 CPU (25 RPS на 1 CPU)

| Сервис                   | 	RPS    | 	Коэффициент (RPS/1000) | Коэффициент с запасом | RAM (Гб) | Network (Гб/с)    | 	CPU       |
|--------------------------|---------|-------------------------|-----------------------|----------|-------------------|------------|
| Session                  | 6 030   | 	6.03                   | 6.5                   | 0.127    | 	    2.5          | 65         |
| Profile                  | 	4      | 	0.004                  | 0.05                  | 0.005    | 	0.004            | 1          |
| SallerProduct            | 	486    | 	0.49                   | 1                     | 0.098    | 	0.0056           | 20         |
| Comment                  | 	1 944  | 	1.94                   | 2.5                   | 0.244    | 	0.432            | 50         |
| ShoppingCart             | 	3 111  | 	3.1                    | 3.5                   | 0.342    | 	1.832            | 70         |
| OrderItem                | 	738    | 	0.74                   | 1                     | 0.098    | 	0.24             | 20         |
| Product                  | 	11 668 | 	11.67                  | 12                    | 1.17     | 	1.84             | 240        |
| Search                   | 	1 555  | 	1.56                   | 2                     | 1.95     | 	0.0156           | 80         |
| AppIndever               | 	486    | 	0.49                   | 1                     | 0.098    | 	0.0056           | 20         |
| ProxyServer Nginx&&Kafka | 	19 505 | 	19.5                   | 20                    | 0.39     | 	4.37             | 200        |


2. При выборе общей конфигурации сервисов
* По данным с конференции Saint HighLoad++ 2024, для сервисов на Golang среднее значение RPS на 1 CPU может составлять примерно 50 RPS [[38]](https://highload.ru/spb/2024/abstracts/11368)
...
* Для начала заметим что некоторый сервисы можно развернуть по парно на общих серверах (для экономии). К примеру можно собрать следующие пары: 
  * (Profile и Comment) - занимают 51 CPU и 0.436 Гб RAM
  * (SallerProduct и OrderItem) - занимают 40 CPU и 0.2456 Гб RAM
* ShoppingCart и AppIndever выделим по одному серверу
* Session и Search занимает больше места, поэтому на каждый сервис выделим два сервера 
* Большая нагрузка ляжет на сервисы Product и ProxyServer Nginx&&Kafka на них выделим по 4 сервера
С учетом 2 реплик на каждый сервис в 4 ДЦ получаем: 16*2*4

| Название | Хостинг | Конфигурация [[40]](https://www.team.ru/server/server_intel_builder.php?model=R2000CY&platform=M50CYP2UR312)                                                                                                                                                                                                                                            | Cores | Cnt | Покупка (руб) | Аренда (месяц руб)                     |
| -------- |---------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| ----- |-----|---------------|----------------------------------------|
| kubenode | У себя  | Платформа: 2x 3G Xeon Scalable(270W)/32D, 12x3.5/2.5 SATA/SAS/NVMe(4), 8xPCIe, 1300/1600/2100W(0/2) <br/> Процессоры: Xeon Gold     6338N  2.2/3.5 GHz, 32C/64T, 48 MB L3, DDR4-2667/6.0 TB, 11.2 GT/s UPI(3), 185 W  <br/> Оперативная память:  32 GB Kingston DDR4-3200 Single Rank x4 CL22 1.2v ECC Registered   DIMM, Server Premier (637 500 руб.) |    64 | 128 | 81 600 000    | 9 600 000 руб (75 000 руб на 1 сервер) | 
...
* MongoDB - для MongoDB потребуется кластер из 48 серверов (6 master + 18 реплики).
* AWS S3 - 100 серверов с дисками объемом 10 ТБ.
* InfluxDB - для InfluxDB потребуется кластер из 20 серверов (4 master + 16 реплик)

База данных   | 	Серверы | 	CPU (на сервер) | 	RAM (на сервер) | 	ROM (на сервер) | Итого серверов с резервированием | Итого серверов на все ДЦ 
------------- |----------|------------------|------------------|------------------|----------------------------------|--------------------------|
Redis         | 	9       | 	2               | 	32 GB           | 20 GB            | 18                               | 72                       |
ClickHouse    | 	8       | 	16              | 	64 GB           | 	10-20 TB        | 16                               | 64                       |
Elasticsearch | 	7       | 	16              | 	64 GB           | 	2 TB            | 14                               | 56                       |
Kafka         | 	12      | 	16              | 	64 GB           | 	2 TB            | 24                               | 96                       |
MongoDB       | 	48      | 24               | 	64 GB           | 	5 TB            | 96                               | 384                      |
AWS (S3)      | 100      | 64               | 512 GB           | 100 TB           | 200                              | 800                      |
InfluxDB      | 20       | 32               | 128 GB           | 10 TB            | 40                               | 160                      |
Итого         | 204      | -                | -                | -                | 408                              | 1632                     |
